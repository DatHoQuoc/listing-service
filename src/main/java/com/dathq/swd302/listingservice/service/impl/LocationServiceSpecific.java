...
public class LocationServiceSpecific {
    ...
    private NearbyItem toWardNearbyItem(Object[] row) {
        // row: ward_id, name, code, province_id, ..., distance (last)
        UUID   id       = UUID.fromString(row[0].toString());
        String name     = (String) row[1];
        int    distance = ((Number) row[row.length - 1]).intValue();
        return new NearbyItem(LocationType.WARD, id, name + " Ward", null,
                name + ", Vietnam", 0, 0, distance);
    }

    private NearbyItem toPoiNearbyItem(Object[] row) {
        // row: poi_id, listing_id, name, category, geolocation, distance_meters, created_at, distance
        UUID        id       = UUID.fromString(row[0].toString());
    ...
    private List<String> highlight(String name, String q) {
        if (name.toLowerCase().contains(q.toLowerCase())) return List.of(q);
        return List.of();
    }
